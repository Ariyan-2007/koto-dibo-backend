...
public class ContributionServiceTests
{
    ...
    public async Task DeleteAsync_LegacyCancelledRow_StillDeletes()
    {
        ...
        await _sut.DeleteAsync("household-1", "payer-1", "contribution-1", CancellationToken.None);

        _contributions.Verify(x => x.DeleteAsync("contribution-1", It.IsAny<CancellationToken>()), Times.Once);
    }

    [Fact]
    public async Task DeleteAsync_ByOwner_RemovesContribution()
    {
        var contribution = ActiveContribution(contributedByUserId: "payer-1");
        GivenExistingContribution(contribution);
        GivenMembership(Membership(HouseholdRole.Member, "payer-1"));
    ...
    [Fact]
    public async Task DeleteAsync_AutoGeneratedFromBazar_ThrowsDomainException()
    {
        var contribution = ActiveContribution(contributedByUserId: "payer-1");
        contribution.SourceType = ContributionSourceType.AutoFromBazar;
        contribution.SourceBazarPurchaseId = "purchase-1";
        ...
        var act = () => _sut.DeleteAsync("household-1", "payer-1", "contribution-1", CancellationToken.None);

        await act.Should().ThrowAsync<DomainException>();
        _contributions.Verify(x => x.DeleteAsync(It.IsAny<string>(), It.IsAny<CancellationToken>()), Times.Never);
    }
}
