...
public class ContributionService : IContributionService
{
    ...
    public async Task<ContributionDto> CancelAsync(string householdId, string callerUserId, string contributionId, CancellationToken cancellationToken = default)
    {
        var membership = await _access.RequireMembershipAsync(householdId, callerUserId, HouseholdPermission.ViewContributions, cancellationToken);
        var contribution = await GetOwnedContributionAsync(householdId, contributionId, cancellationToken);

        BazarPurchaseService.RequireEditAccess(membership.Role, contribution.ContributedByUserId, callerUserId, HouseholdPermission.CancelContribution, "contribution");
        RequireActive(contribution);
        RequireNotAutoGenerated(contribution);
    ...
    // cancelling one directly here would desync it from the purchase it mirrors. The purchase is
    // the source of truth; changes flow through BazarPurchaseService instead.
    private static void RequireNotAutoGenerated(Contribution contribution)
    {
        ...
        {
            throw new DomainException("This contribution was auto-generated from a Bazar purchase. Edit or cancel that Bazar purchase instead.");
        }
    }
